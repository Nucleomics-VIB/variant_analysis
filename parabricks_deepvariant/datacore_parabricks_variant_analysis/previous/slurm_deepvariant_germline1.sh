...
#SBATCH --job-name="parabricks_deepvariant_germline_${pfx}"
#SBATCH --output="%x_%j_${pfx}.out"
#SBATCH --error="%x_%j_${pfx}.err"
#SBATCH --nodes=1
#SBATCH --ntasks=32
...
singularity run --nv \
    ...
    --tmp-dir "/tmp"
    #\
    #  > "${WORKDIR}/${outlogs}/${pfx}_deepvariant_germline_XYhap_log.txt" 2>&1

# !!!! remove the last redirect if running as a slurm sbatch job
